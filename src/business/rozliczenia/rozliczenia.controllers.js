...
export const getRozliczeniaMedycynaPracy = id => async (req, res) => {
  try {
    let wizyty = await Wizyta.find({ typWizyty: 'MEDYCYNA_PRACY' }).populate({
      path: 'pacjent',
      model: Pacjent,
      populate: {
        path: 'firma',
    ...
    mapToRozliczeniaMedycynaPracyDto(firmy, wizyty).then(results => res.status(200).json(results));
  } catch (e) {
    console.error(e);
    res.status(400);
  }
...
export const getRozliczeniaSpecjalistyka = () => async (req, res) => {
  try {
    let wizyty = await Wizyta.find({ typWizyty: 'SPECJALISTYKA' }).populate({
      path: 'pacjent',
      model: Pacjent,
      populate: {
        path: 'firma',
    ...
    mapToRozliczeniaSpecjalistyka(wizyty).then(results => res.status(200).json(results));
  } catch (e) {
    console.error(e);
    res.status(400);
  }
